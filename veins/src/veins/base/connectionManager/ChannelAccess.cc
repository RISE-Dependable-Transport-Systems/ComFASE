...
simtime_t ChannelAccess::calculatePropagationDelay(const NicEntry* nic)
{
    ...
    if (comfase->delayAttack){
        ...
        return comfase->comfase_delayAttack(senderModule->getId(), receiverModule->getId(), correctValue);
    }
    else if (comfase->DoSAttack){
        //std::cout<<"DoS Attack = is TRUE"<<std::endl;
        float correctValue = receiverPos.distance(senderPos) / BaseWorldUtility::speedOfLight();
        return comfase->comfase_DoSAttack(senderModule->getId(), receiverModule->getId(), correctValue);
    }
    else{
