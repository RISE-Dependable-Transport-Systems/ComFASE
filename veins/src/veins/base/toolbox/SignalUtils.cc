...
double VEINS_API getMinSINR(simtime_t start, simtime_t end, AirFrame* signalFrame, AirFrameVector& interfererFrames, double noise)
{
    ...
    int receiverID = signalFrame->getArrivalModuleId();
    int senderID = signalFrame->getSenderModuleId();
    std::cout<<"sender module ID= "<<senderID<<"-----receiver= "<<receiverID<<std::endl;
    ...
    if (comfase->deceptiveJammingAttack){
        //std::cout<<"Deceptive Jamming Attack = is Active"<<std::endl;
        float injectedInterferenceValue = comfase->comfase_deceptiveJammingAttack(senderID, receiverID, interference.at(7));
        interference.at(6) = injectedInterferenceValue;
        interference.at(7) = injectedInterferenceValue;
        interference.at(8) = injectedInterferenceValue;
    }
    if (comfase->barrageJammingAttack){
       //std::cout<<"Barrage Jamming Attack = is TRUE"<<std::endl;
       noise = comfase->comfase_barrageJammingAttack(27, 37, noise);
    }
       //std::cout<<"InterfererFrames value is= "<<interfererFrames<<" at time: "<<simTime()<<std::endl;
    Signal sinr = signal / (interference + noise);
    std::cout<<"SINR - Signal value in 8 is="<<signal.at(7)<<std::endl;
