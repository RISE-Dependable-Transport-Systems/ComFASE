...
void BasePhyLayer::filterSignal(AirFrame* frame)
{
    ...
    if (comfase->destructiveInterferenceAttack){
        //std::cout<<"Destructive Interference Attack "<<std::endl;
        float injectedDestructiveness = comfase->comfase_destructiveInterferenceAttack(senderID, receiverID, signal.at(7));
        signal.at(6) *= injectedDestructiveness;
        signal.at(7) *= injectedDestructiveness;
        signal.at(8) *= injectedDestructiveness;
    }
...
double BasePhyLayer::getNoiseFloorValue()
{
    ...
    //if (comfase->NoiseAttack){
      ...
      // return comfase->BarrageJammingAttack(27, 37, noiseFloorValue);
    //}
    //else{
